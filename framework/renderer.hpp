...
public:
  ...
  Color getDiffuseIllumination(Hit const& hit, vec3 const& normHit, Scene const& scene, shared_ptr<Shape> const& nearestObject, shared_ptr<Light> const& light);
  ...
  Color ptLight(Hit const &hit, vec3 const& normHit, Ray const &ray, Scene const& scene, shared_ptr<Shape> const& nearestObject);
